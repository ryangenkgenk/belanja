...
const UpcomingEvents = ({ events }) => {
	const sportsEvents = events?.filter((evt) => evt.attributes.category === 'sports');
	const corporateEvents = events?.filter((evt) => evt.attributes.category === 'corporate');
	const privateEvents = events?.filter((evt) => evt.attributes.category === 'private');
	const charityEvents = events?.filter((evt) => evt.attributes.category === 'charity');
	const festivalEvents = events?.filter((evt) => evt.attributes.category === 'festival');
	const concertEvents = events?.filter((evt) => evt.attributes.category === 'concert');
	const partyEvents = events?.filter((evt) => evt.attributes.category === 'party');
								...
								<div key={evt.id} className='col-md-6 col-lg-4 mb-4'>
									<div className='upcoming-events__item'>
										<div className='image'>
											...
											{evt?.attributes?.eventType !== 'none' ? <div className='popular'>{evt?.attributes?.eventType}</div> : ''}
										</div>
